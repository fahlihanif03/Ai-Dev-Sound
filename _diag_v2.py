...
def audio_to_features_v2(y, sr=SAMPLE_RATE):
    ...
    for i in range(n_frames):
        ...
        ref = mel_energy.max() + 1e-8
        normalized = mel_energy / ref
        out[:, i] = np.log(normalized + 1.0)
    return out


def vectors_per_window_v2(seconds, sr=SAMPLE_RATE):
    y = np.zeros(int(seconds * sr), dtype=np.float32)
    feats = audio_to_features_v2(y, sr)
...
window_size = vectors_per_window_v2(WINDOW_SECONDS)
...
print(f"\n[DIAGNOSTIC v2 - power + per-frame max-ref] AUC: {roc_auc_score(labels, scores)*100:.2f}%  pAUC: {roc_auc_score(labels, scores, max_fpr=0.1)*100:.2f}%")
