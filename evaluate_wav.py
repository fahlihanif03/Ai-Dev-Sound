...
def load_model_and_stats(machine_id):
    ...
    if "err_std" not in stats:
        raise SystemExit(
            ...
            f"--machine_id {machine_id} to regenerate it."
        )
    return model, stats["mean"], stats["std"], stats["err_std"], float(stats["threshold"])


def score_wav(path, model, mean, std, err_std):
...
def main():
    ...
    model, mean, std, err_std, saved_threshold = load_model_and_stats(args.machine_id)
    ...
    for path in args.wav_files:
        score = score_wav(path, model, mean, std, err_std)
